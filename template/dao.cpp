...
#define gcd __gcd
    ...
    /* ģ\CA\FD\C1\BD\B1\B6\B8պó\AC int ʱ\A1\A3
...
map<int, Int> f[2]; Int d;
Int ff[N+1][N+1], gg[N+1], g[N+1], z[N+1][N+1];
int p, q;
...
int main(){
    ...
    n = N; REP_1(m, N){
        ...
        REP_1(i, m){
            ...
            REP(j, m){
                ...
                ECH(it, f[q]){
                    d = it->se; int s = it->fi;
                    if(j && !_1(s, j) && _1(s, j-1)) f[p][s^_1(j-1)] += d;
                    f[p][s^_1(j)] += d;
                }
            }
    ...
    REP_1(m, N){
        REP(s, _1(m-1)){
            ...
            REP_1(i, m){
                g[i] = gg[i]; REP_1(j, i-1) g[i] -= g[j] * gg[i-j];
